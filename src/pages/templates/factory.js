...
const Templates = ({DEFAULTCONFIG, FORCEDEFAULT = false}) => {
    ...
    // Display unique or group of information
    const useDefault = (getTemplates && getTemplates.length === 0) || FORCEDEFAULT;

    //Display the correct option
    useEffect( () => {
        if (!useDefault && getTemplates) {
            setOptions(getTemplates[0].config);
        }
    }, [useDefault,getTemplates])


    if (loading) return <Loader/>;
    //validate min information
    ...
    return ( 
        <>
        {valid ?
            <Grid stackable className="flex flex-col-reverse w-full">
            <Grid.Column width={5}  >  
                <Card fluid>
                    <Card.Content>
                            ...
                            <Options 
                                ...
                                options={options} 
                                TemplateData={useDefault? [DEFAULTCONFIG] : getTemplates} 
